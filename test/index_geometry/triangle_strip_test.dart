    ...
    group('default constructor', () {
      ...
      group('with a negative offset', () {
        test('throws a RangeError', () {
          expect(() => new TriangleStrip(vertices, indices, -1), throwsRangeError);
        });
      });

      group('with a offset equal to the length of the list of indices', () {
        test('throws a RangeError', () {
          expect(() => new TriangleStrip(vertices, indices, 8), throwsRangeError);
        });
      });

      group('with a valid offset and a negative count', () {
        test('throws a RangeError', () {
          expect(() => new TriangleStrip(vertices, indices, 1, -1), throwsRangeError);
        });
      });
      ...
      group(
        ...
        test('throws a RangeError', () {
          expect(() => new TriangleStrip(vertices, indices, 1, 8), throwsRangeError);
        });
      });
      ...
      group('with a valid offset and count', () {
        group('with a count smaller than 3', () {
          test('returns an instance with a length of 0', () {
            expect(new TriangleStrip(vertices, indices, 1, 2).length, equals(0));
          });
        });

        group('with a count of 2', () {
          test('returns an instance with a length of 3', () {
            expect(new TriangleStrip(vertices, indices, 1, 3).length, equals(1));
          });
        });
      });
    });
